    ...
    <>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <Panel count={data.orders.length} title="Orders">
          {data.orders.length === 0 ? <Empty /> : data.orders.map((row) => <a className="flex min-h-20 flex-col items-start justify-between gap-3 px-5 py-4 transition-[background-color] duration-[var(--creatorjobs-motion-fast)] ease-[var(--creatorjobs-motion-ease)] hover:bg-foreground/[0.025] sm:flex-row sm:items-center" href={`/orders/${row.id}`} key={String(row.id)}><div className="min-w-0"><p className="break-words font-medium">{String(row.title)}</p><p className="mt-1"><Meta>{shortId(row.whop_payment_id)} · {String(row.buyer)}</Meta></p></div><div className="flex w-full items-center justify-between gap-3 sm:w-auto sm:block sm:text-right"><Badge tone={statusTone(row.status)}>{String(row.status)}</Badge><p className="text-sm font-semibold sm:mt-2" data-numeric>{money(Number(row.amount_cents), String(row.currency))}</p></div></a>)}
        </Panel>
