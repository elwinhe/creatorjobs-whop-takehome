    ...
    <>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <Panel count={data.sellers.length} title="Sellers">
          {data.sellers.length === 0 ? <Empty /> : data.sellers.map((row) => <div className="flex min-h-20 flex-col items-start justify-between gap-3 px-5 py-4 sm:flex-row sm:items-center" key={String(row.id)}><div className="min-w-0"><p className="break-words font-medium">{String(row.display_name)}</p><p className="mt-1"><Meta>{shortId(row.whop_company_id)} · {row.has_payout_method ? 'PAYOUT READY' : 'PAYOUT PENDING'}</Meta></p></div><div className="flex w-full flex-wrap items-center justify-between gap-2 sm:w-auto sm:justify-end"><Badge tone={row.onboarding_status === 'payout_ready' ? 'success' : 'neutral'}>{String(row.onboarding_status)}</Badge><Button aria-label={`Regenerate account link for ${row.display_name}`} disabled={busySeller === row.id || !row.whop_company_id} onClick={() => regenerate(String(row.id))} size="sm" variant="ghost">{busySeller === row.id ? <LoaderCircle className="size-4 animate-spin" /> : <ArrowUpRight className="size-4" />} Link</Button></div></div>)}
        </Panel>
